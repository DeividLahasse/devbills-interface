import type { CreateTransactionDTO, MonthlyItem, TranasactionFilter, Transaction, TransactionSummary } from "../types/transactions";
import { api } from "./api";

export const getTransactions = async (filter?: Partial<TranasactionFilter>): Promise<Transaction[]> => {
  const reponse = await api.get<Transaction[]>("/transactions", {
    params: filter,
  });
...
export const createTrasaction = async(transactionData:CreateTransactionDTO):Promise<Transaction> =>{
